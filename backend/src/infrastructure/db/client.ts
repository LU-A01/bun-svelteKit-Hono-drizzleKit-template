...
console.log(`環境変数ファイルパス: ${envPath} (存在: ${fs.existsSync(envPath) ? "はい" : "いいえ"})`);
...
try {
  ...
  console.log(`- タイプ: ${isTurso ? 'Turso' : isSQLite ? 'SQLite' : isMemory ? 'メモリ' : '不明'}`);
  console.log(`- URL: ${url}`);
  ...
  // LibSQLクライアントの作成
  console.log("接続設定:", JSON.stringify(connectionConfig, (key, value) => key === 'authToken' ? '***' : value));
  ...
  (async () => {
    try {
      ...
      console.warn("警告: データベースへの接続に問題がありますが、アプリケーションの実行を継続します。");
    }
  })();
} catch (error) {
  console.error("データベース接続エラー:", error);
